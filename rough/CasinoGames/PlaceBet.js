...
const PlaceBet = ({
  ...
  const loadOpenBets = useCallback(async () => {
    if (gameType !== "odi_teenpatti") return;

    try {
      const currentEventId = betData?.eventId || betData?.roundId || "1";
      ...
      console.log("ODITEENPATTI OpenBets Response:", res);
  ...
  const formatResultData = useCallback((result) => {
    ...
    if (gameType === "odi_teenpatti") {
      ...
      // Default behavior: first 3 for A, next 3 for B
  ...
  useEffect(() => {
    const loadResults = async () => {
      try {
        ...
        const response = await fetchTeenpattiResults(eventId, "teen");
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      console.log("📤 Emitting 'gameResult' to request history...");
  ...
  const handleSpanClick = async (clickedIndex, e) => {
    ...
    try {
      ...
      const response = await fetchTeenpattiResults(mid, "teen", 1, login_user_id, auth_key);
  ...
  const handleSubmit = async () => {
    ...
    if (betAmount >= minBet && betAmount <= maxBet) {
      ...
      params.append("eventType", "ODITEENPATTI");
      ...
      params.append("bet_market_type", "ODITEENPATTI");
      params.append("oddsmarketId", betData.marketId);
      params.append("eventManualType", "Auto");
      params.append("market_runner_name", teamName);
      params.append("market_odd_name", "ODITEENPATTI");
      params.append("bet_event_name", "ODITEENPATTI");
      ...
      // Get event ID - prioritize mid from socket data
      let eventId = null;

      // First try to get mid from socket data
      if (socketRef.current?.eventData?.mid) {
        eventId = socketRef.current.eventData.mid;
        console.log("✅ Using mid from socket data:", eventId);
      }
      // Then try event_id from socket data
      else if (socketRef.current?.eventData?.event_id) {
        eventId = socketRef.current.eventData.event_id;
        console.log("ℹ️ Using event_id from socket data:", eventId);
      }
      // Then try from betData
      else if (betData?.roundId) {
        eventId = betData.roundId;
        console.log("ℹ️ Using roundId from betData:", eventId);
      } else if (betData?.eventId) {
        eventId = betData.eventId;
        console.log("ℹ️ Using eventId from betData:", eventId);
      } else if (betData?.id) {
        eventId = betData.id;
        console.log("ℹ️ Using id from betData:", eventId);
      } else {
        console.warn("⚠️ No valid event ID found in socket or betData");
      }
      ...
      const socketGameType = socketRef.current?.eventData?.gtype || "teen";
      params.append("gtype", socketGameType);
      console.log("🎮 Game type being sent:", socketGameType);
      ...
      try {
        const response = await placeBetApi(params);
        if (isApiSuccess(response)) {
          ...
          // Optimistically update OpenBets for immediate UI feedback (Odi Teen Patti)
          if (gameType === "odi_teenpatti") {
            const newOpenBet = {
              bet_id: newBet.id,
              market_name: newBet.market_name,
              bet_type: newBet.bet_type,
              bet_odds: newBet.odds,
              bet_stack: newBet.stake
            };
            setOpenBets(prev => [newOpenBet, ...prev]);
          }

          toast.success(response?.message || "Bet placed successfully!");

          if (gameType === "odi_teenpatti") {
            refreshBalanceApi(dispatch, { curPageName: "live_odi_teenpatti.php" });
  ...
  // ------------------------------------------------------------------
  // MOBILE ODI TEEN PATTI RENDER
  // ------------------------------------------------------------------
  if (betData && gameType === "odi_teenpatti" && isMobile) {
    return (
      <div style={{
  ...
  return (
    <>
      ...
      {/* MY BETS SECTION - Only for ODI Teenpatti and if bets exist */}
      {gameType === "odi_teenpatti" && openBets.length > 0 && (
        <div className="mt-2">
          <div
            style={{
              maxWidth: "375px",
      ...
      >
        ...
        >
          ...
          >
            Teenpatti 1-day Result111111111
          </span>
      ...
      {hasPlacedBet && gameType !== "odi_teenpatti" && (
        <>
