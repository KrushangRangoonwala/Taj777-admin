...
const PlacebetInstantTeenPatti = ({
  betData,
  onSubmit,
  onClose,
  hideResults,
  ...
  const loadOpenBets = useCallback(async () => {
    try {
      const res = await fetchOpenBetsApi({
        markettype: "INSTANTTEENPATTI",
        eventId: "1",
        curPageName: "live_instant_teenpatti.php",
      });
  ...
  useEffect(() => {
    const loadResults = async () => {
      try {
        ...
        const response = await fetchTeenpattiResults(eventId, "teen3");
    ...
    socket.on("game", (data) => {
      ...
      if (mid && socketRef.current.lastMid !== mid) {
        ...
        if (socketRef.current.lastMid) {
          ...
          console.log("🔄 Round changed (mid: " + mid + "), requesting gameResult...");
          socket.emit("gameResult");
        }
  ...
  const handleSpanClick = async (clickedIndex, e) => {
    ...
    try {
      ...
      const response = await fetchTeenpattiResults(mid, "teen3", 1, login_user_id, auth_key);
  ...
  const handleSubmit = async () => {
    ...
    if (betAmount >= minBet && betAmount <= maxBet) {
      ...
      params.append("eventType", "INSTANTTEENPATTI");
      ...
      params.append("bet_market_type", "INSTANTTEENPATTI");
      params.append("oddsmarketId", betData.marketId);
      params.append("eventManualType", "Auto");
      params.append("market_runner_name", teamName);
      params.append("market_odd_name", "INSTANTTEENPATTI");
      params.append("bet_event_name", "INSTANTTEENPATTI");
      ...
      try {
        const response = await placeBetTeen3Api(params);
        if (isApiSuccess(response)) {
          ...
          refreshBalanceApi(dispatch, { curPageName: "live_instant_teenpatti.php" });
          setTimeout(() => {
            loadOpenBets();
...
export default PlacebetInstantTeenPatti;
