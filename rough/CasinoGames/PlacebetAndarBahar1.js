...
const PlaceBet = ({
  ...
  useEffect(() => {
    ...
    socket.on("ab20", (data) => {
      console.log("📥 Andar Bahar 20 Game Result Data:", data);
  ...
  const handleSpanClick = async (item) => {
    try {
      ...
      if (response && response.length > 0) {
        setModalContent({ data: JSON.stringify(response) });
        setIsModalOpen(true);
      } else {
  ...
  const handleSubmit = async () => {
    ...
    if (betAmount >= minBet && betAmount <= maxBet) {
      ...
      try {
        // Use the new API
        const response = await placeBetAb20Api(params);
        if (isApiSuccess(response)) {
          ...
          refreshBalanceApi(dispatch, { curPageName: "live_ab20.php" });
    ...
    <>
      {!hideResults && (
        <div className="casino-place-bet">
          ...
          <div className="casino-video-last-results">
            ...
            <a href="/report/casinoresult/ab20" className="result-more">
              ...
            </a>
          </div>
        </div>
              ...
              >
                ...
                <tbody>
                  <tr>
                    ...
                    >
                      {formatBetTitle(betData.teamName, betData.side)}
                    </td>
      ...
      {betData && isMobile ? (
        ...
        >
          ...
          >
            <span>{formatBetTitle(betData.teamName, betData.side)}</span>
      ...
      >
        ...
        >
          <span>Round ID: {modalContent?.roundId || (modalContent?.data && JSON.parse(modalContent.data)[0]?.mid)}</span>
          <span>Match Time: {modalContent?.matchTime || (modalContent?.data && new Date(JSON.parse(modalContent.data)[0]?.time).toLocaleString())}</span>
        </div>
