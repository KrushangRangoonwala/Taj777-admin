...
const InstantWorli = ({ onBetSelection, lastBetTime, exposureTrigger }) => {
    ...
    const fetchExposure = async () => {
      ...
      try {
        const response = await fetchCasinoExposureApi({
          ...
          curPageName: "live_instant_worli.php",
        });
    ...
    return (
      <div
        ...
        onClick={() => {
          if (!isSuspended) {
            setSelectedSid(sid);
            handleOddsClick(teamName, 9, sid);
          }
        }}
        style={{ cursor: !isSuspended ? "pointer" : "default", backgroundColor: selectedSid === sid ? "#03b37f" : "", }}
      >
        <span className="worli-odd">
          {label}
        </span>

        {/* ✅ extra line only for Line / Odd / Even */}
        {extraTextMap[label] && (
    ...
    <>
      ...
      <div className="casino-table worli kk">
        <CasinoVideo
          ...
          videoSrc="/mediaplayer/instant_worli/95bba40e-0623-4fe3-a007-0b71f467e211"
        ...
        <div className="casino-detail">
          <div className="casino-box tab-content">
            <div className="tab-pane active single">
              <div className={`worlibox ${isWorliSuspended() ? "suspended" : ""}`}>
                {/* LEFT */}
                <div className="worli-left">
                  <div className="worli-box-title"><b>9</b></div>

                  <div className="worli-box-row">
                    {[1, 2, 3, 4, 5].map((n) => renderWorliBox(n, n))}
                  </div>

                  <div className="worli-box-row">
                    {[6, 7, 8, 9, 0].map((n) => renderWorliBox(n, n))}
                  </div>
                </div>
                ...
                <div className="worli-right">
                  ...
                  <div className="worli-box-row">
                    {renderWorliBox("Line 1", "LINE1")}
                    {renderWorliBox("Odd", "ODD")}
                  </div>

                  <div className="worli-box-row">
                    {renderWorliBox("Line 2", "LINE2")}
                    {renderWorliBox("Even", "EVEN")}
                  </div>
                </div>
              </div>
            </div>
