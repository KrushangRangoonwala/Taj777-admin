...
const PlacebetTeen20c = ({
  ...
  const loadOpenBets = useCallback(async () => {
    try {
      const res = await fetchOpenBetsApi({
        markettype: "2020TEENPATTIC",
        eventId: "1",
        curPageName: "live_teenpattit20c.php",
      });
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      console.log("✅ PlacebetTeen20c connected to result socket:", socket.id);
  ...
  const handleSubmit = async () => {
    ...
    if (betAmount >= minBet && betAmount <= maxBet) {
      ...
      params.append("eventType", "2020TEENPATTIC");
      ...
      params.append("bet_market_type", "2020TEENPATTIC");
      params.append("oddsmarketId", betData.marketId);
      params.append("eventManualType", "Auto");
      params.append("market_runner_name", teamName);
      params.append("market_odd_name", "2020TEENPATTIC");
      params.append("bet_event_name", "2020TEENPATTIC");
      ...
      try {
        const response = await placeBetTeen20cApi(params);
        if (isApiSuccess(response)) {
          toast.success(response?.message || "Bet placed successfully!");
          refreshBalanceApi(dispatch, { curPageName: "live_teenpattit20c.php" });
          setTimeout(() => {
            loadOpenBets();
...
export default PlacebetTeen20c;
