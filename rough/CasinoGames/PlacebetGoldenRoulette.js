...
const PlacebetGoldenRoulette = ({
  ...
  const loadOpenBets = useCallback(async () => {
    try {
      const res = await fetchOpenBetsApi({
        markettype: "ROULETTE11", // Updated for Beach Roulette
        eventId: "1",
        curPageName: "live_roulette11.php", // Placeholder page name
      });
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      socket.emit("Room", "roulette11"); // Updated room name
    ...
    socket.on("gameResult", (data) => {
      ...
      if (data?.res?.length > 0) {
        ...
        socketRef.current.eventData = {
          ...
          gtype: latestResult.gtype || "roulette11", // Updated game type
        };
      }
  ...
  const handleSpanClick = async (clickedIndex, e) => {
    ...
    try {
      ...
      const response = await fetchTeenpattiResults(
        mid,
        resultItem.gtype || "roulette11", // Updated game type
        1,
        login_user_id,
        auth_key
      );
  ...
  const handleSubmit = async () => {
    ...
    params.append("eventType", "ROULETTE11");
    ...
    try {
      ...
      const response = await placeBetRoulette11Api(params);
...
export default PlacebetGoldenRoulette;
