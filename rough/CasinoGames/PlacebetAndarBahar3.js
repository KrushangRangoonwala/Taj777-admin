...
const PlaceBet = ({
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      console.log("🎮 Joining 'ab3' room...");
      socket.emit("Room", "ab3");
      console.log("📤 Emitting 'ab3' to request data...");
      socket.emit("ab3");
    });

    // Listen for game results
    socket.on("ab3", (data) => {
      console.log("📥 Andar Bahar 3 Game Result Data:", data);
  ...
  const handleSpanClick = async (item) => {
    try {
      ...
      if (response && response.length > 0 && response[0]) {
        ...
        console.log("Andar Bahar 3 API Data:", apiData);
  ...
  const handleSubmit = async () => {
    ...
    if (betAmount >= minBet && betAmount <= maxBet) {
      ...
      try {
        // Use the new API
        const response = await placeBetAb3Api(params);
        if (isApiSuccess(response)) {
          ...
          refreshBalanceApi(dispatch, { curPageName: "live_ab3.php" });
