...
const PlacebetBeachRoulette = ({
  ...
  const loadOpenBets = useCallback(async () => {
    try {
      const res = await fetchOpenBetsApi({
        markettype: "ROULETTE12", // Updated for Beach Roulette
        eventId: "1",
        curPageName: "live_roulette12.php", // Placeholder page name
      });
  ...
  const formatResultData = useCallback((result) => {
    ...
    // Adapt logic for Beach Roulette
    playerACards = [];
    playerBCards = [];
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      console.log(
        "✅ PlacebetBeachRoulette connected to result socket:",
        socket.id
      );
      socket.emit("Room", "roulette12"); // Updated room name
    ...
    socket.on("gameResult", (data) => {
      ...
      if (data?.res?.length > 0) {
        ...
        socketRef.current.eventData = {
          ...
          gtype: latestResult.gtype || "roulette12", // Updated game type
        };
      }
  ...
  const handleSpanClick = async (clickedIndex, e) => {
    ...
    try {
      ...
      const response = await fetchTeenpattiResults(
        mid,
        resultItem.gtype || "roulette12", // Updated game type
        1,
        login_user_id,
        auth_key
      );
  ...
  const handleSubmit = async () => {
    ...
    params.append("eventType", "ROULETTE12");
    ...
    try {
      ...
      const response = await placeBetRoulette12Api(params);
          ...
          <div className="casino-place-bet">
            ...
            >
              {lastResults && lastResults.length > 0 ? (
                <>
                  {lastResults.slice(0, 9).map((result, index) => {
...
export default PlacebetBeachRoulette;
