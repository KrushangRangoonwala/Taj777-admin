...
const PlacebetAAA = ({
  ...
  const loadOpenBets = useCallback(async () => {
    if (gameType !== "aaa" && gameType !== "odi_teenpatti") return;
  ...
  useEffect(() => {
    const loadResults = async () => {
      try {
        ...
        const response = await fetchTeenpattiResults(formattedEventId, "aaa");
  ...
  const handleSubmit = async () => {
    ...
    if (betAmount >= minBet && betAmount <= maxBet) {
      ...
      if (eventId && eventId.toString().includes(".")) {
        eventId = eventId.toString().split(".")[1];
        console.log("✂️ Processed eventId (removed prefix):", eventId);
      }

      const params = new URLSearchParams();
      params.append("eventId", eventId || betData.eventId); // Use processed eventId, fallback to betData
      params.append("eventType", "AMAR_AKBAR_ANTHONY");
      ...
      params.append("bet_market_type", "AMAR_AKBAR_ANTHONY");
      params.append("oddsmarketId", betData.marketId);
      params.append("eventManualType", "Auto");
      params.append("market_runner_name", teamName);
      params.append("market_odd_name", "AMAR_AKBAR_ANTHONY");
      params.append("bet_event_name", "AMAR_AKBAR_ANTHONY");
      ...
      try {
        const response = await placeBetAAAApi(params);
