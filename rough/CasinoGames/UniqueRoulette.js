...
import {
  ...
  placeBetUniqueRouletteApi,
  betActionUniqueRouletteApi,
  fetchUniqueRouletteBetsApi,
  fetchUniqueRouletteResultsApi,
  getDefaultParams,
  isApiSuccess,
  fetchOpenBetsApi,
  refreshBalanceApi,
...
import "./UniqueRoulette.css";
import RulesModal from "../Modal/RulesModal";
import { GameRules } from "./components/GameRules";
import { useGetFileData } from "../../hooks/useGetFileData";

const UniqueRoulette = ({ onBetSelection, lastBetTime }) => {
    ...
    const fetchExposure = async () => {
      ...
      try {
        const response = await fetchCasinoExposureApi({
          markettype: "OURROULLETE",
          main_event_id: gameData.t1[0].mid,
          curPageName: "live_ourroullete.php",
        });
  ...
  React.useEffect(() => {
    ...
    const joinRoom = () => {
      console.log("📤 Emitting Room: ourroullete");
      socket.emit("Room", "ourroullete");
    };
    ...
    socket.on("game", (data) => {
      console.log("Roulette Socket Data:", data);
    ...
    socket.on("gameResult", (data) => {
      console.log("Roulette Socket Results:", data);
  ...
  const fetchBets = async () => {
    ...
    try {
      ...
      const payload = new URLSearchParams({
        ...authParams,
        eventId: mid,
        eventType: "OURROULLETE",
      });
      const data = await fetchUniqueRouletteBetsApi(payload);
  ...
  const handleBetAction = async (actionType) => {
    ...
    try {
      ...
      const data = await betActionUniqueRouletteApi(payload);
      if (isApiSuccess(data)) {
        if (
          (actionType === "undo" ||
            actionType === "clear" ||
        ...
        loadOpenBets();
        refreshBalanceApi(dispatch, { curPageName: "live_ourroullete.php" });
        fetchBets();
  ...
  React.useEffect(() => {
    if (gameData?.t1?.[0]?.mid) {
      ...
      refreshBalanceApi(dispatch, { curPageName: "live_unique_roulette.php" });
      fetchBets();
    }
  }, [gameData?.t1?.[0]?.mid]);
  ...
  React.useEffect(() => {
    const fetchInitialResults = async () => {
      try {
        ...
        const data = await fetchUniqueRouletteResultsApi(payload);
  ...
  const handleSelection = async (selectionName, sid) => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        odds: rate,
        runs: rate,
        bet_market_type: "OURROULLETE",
        oddsmarketId: sid,
        eventManualType: "Auto",
        market_runner_name: selectionName,
        market_odd_name: "OURROULLETE",
        bet_event_name: "Unique Roulette",
      ...
      const data = await placeBetUniqueRouletteApi(params);
      if (isApiSuccess(data)) {
        if (data.last_bet_id) {
  ...
  const NumberBox = ({ num, isMobileBoard = false }) => {
    ...
    const bgColor = isRed ? "#b2121f" : "#111"; // Red or Black
    ...
    return (
      <div
        style={{
          ...
          justifyContent: "center",
          alignItems: "center",
          backgroundColor: isWinner ? bgColor : isSuspended ? lockedColor : bgColor, // Specific locked colors
          color: isWinner ? "#fdcf13" : isSuspended ? "#000" : "#fff",
          fontWeight: "bold",
          fontSize: isWinner ? (isMobileBoard ? "24px" : "32px") : (isMobileBoard ? "18px" : "18px"),
          border: `1px solid ${isLight ? "#fff" : "#333"}`,
      ...
      >
        {/* Transparent Center Hitbox for reliable single-number betting */}
        <div
          onClick={(e) => {
            e.stopPropagation();
    ...
    return (
      <div
        ...
        style={{
          ...
          fontWeight: "bold",
          fontSize: isMobileBoard ? "19px" : "18px",
          border: `1px solid ${isLight ? "#fff" : "#333"}`,
          cursor: isSuspended ? "not-allowed" : "pointer",
          gridArea: gridArea,
    ...
    <>
        ...
        {isMobile ? (
          <div>
            ...
            >
                      ...
                      style={{
                        ...
                        color:
                          winningNumber !== null &&
                            ...
                            ? "#fdcf13"
                            : isMarketSuspended(getMarket(getSidByNat("0")))
                              ? "#000" // Black text on light background
                              : "#fff",
                        display: "flex",
                        justifyContent: "center",
                        alignItems: "center",
              ...
              <div style={{ marginTop: "35px" }}>
                <RouletteChips
                  ...
                  showRepeat={false} // User requested hidden
                  showClear={false}  // User requested hidden
                />
              </div>
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", marginTop: "5px", marginRight: "5px" }}>
                {/* Statistics Table requested by user */}
                <table
                ...
                >
                  <thead style={{ backgroundColor: "#126e51", color: "#fff" }}>
                    <tr>
                      <th style={{ padding: "4px", fontSize: "12px", textAlign: "center", border: "1px solid #fff", color: "#fff", fontWeight: "bold" }}>Single</th>
                      <th style={{ padding: "4px", fontSize: "12px", textAlign: "center", border: "1px solid #fff", color: "#fff", fontWeight: "bold" }}>Split</th>
                      <th style={{ padding: "4px", fontSize: "12px", textAlign: "center", border: "1px solid #fff", color: "#fff", fontWeight: "bold" }}>Street</th>
                      <th style={{ padding: "4px", fontSize: "12px", textAlign: "center", border: "1px solid #fff", color: "#fff", fontWeight: "bold" }}>Corner</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr style={{ backgroundColor: "#2e3439" }}>
                      <td style={{ padding: "4px", fontSize: "22px", textAlign: "center", border: "1px solid #fff", color: "#a4a8ae" }}><b>0</b></td>
                      <td style={{ padding: "4px", fontSize: "22px", textAlign: "center", border: "1px solid #fff", color: "#a4a8ae" }}><b>0</b></td>
                      <td style={{ padding: "4px", fontSize: "22px", textAlign: "center", border: "1px solid #fff", color: "#a4a8ae" }}><b>0</b></td>
                      <td style={{ padding: "4px", fontSize: "22px", textAlign: "center", border: "1px solid #fff", color: "#a4a8ae" }}><b>0</b></td>
                    </tr>
                  ...
                  return (
                    <div className="rules-body">
                      <div>
                        ...
                        <GameRules normalizedGame={'Unique Roulette'} />
                      </div>
                    </div>
                  );
                })()}
      ...
      >
        ...
        >
          ...
          >
            <span style={{ fontSize: "14px" }}>Unique Roulette Result</span>
            <button
      ...
      <RulesModal
        ...
        gameKey={'Unique Roulette'}   // or gameId
      />
    </>
  );
};
