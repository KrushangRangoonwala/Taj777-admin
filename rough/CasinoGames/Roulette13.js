...
import {
  ...
  betActionRoulette13Api,
  fetchRoulette13BetsApi,
  fetchRoulette13ResultsApi,
  getDefaultParams,
  isApiSuccess,
  fetchOpenBetsApi,
  refreshBalanceApi,
...
import "./Roulette13.css";
import RulesModal from "../Modal/RulesModal";
import { GameRules } from "./components/GameRules";
import { useGetFileData } from "../../hooks/useGetFileData";

const Roulette13 = ({ onBetSelection, lastBetTime }) => {
    ...
    const fetchExposure = async () => {
      ...
      try {
        const response = await fetchCasinoExposureApi({
          markettype: "ROULETTE13",
          main_event_id: gameData.t1[0].mid,
          curPageName: "live_roulette13.php",
        });
  ...
  React.useEffect(() => {
    ...
    const joinRoom = () => {
      console.log("📤 Emitting Room: roulette13");
  ...
  const fetchBets = async () => {
    ...
    try {
      ...
      const data = await fetchRoulette13BetsApi(payload);
  ...
  const handleBetAction = async (actionType) => {
    ...
    try {
      ...
      const data = await betActionRoulette13Api(payload);
      if (isApiSuccess(data)) {
        if (
          (actionType === "undo" ||
            actionType === "clear" ||
  ...
  React.useEffect(() => {
    const fetchInitialResults = async () => {
      try {
        ...
        const data = await fetchRoulette13ResultsApi(payload);
  ...
  const handleSelection = async (selectionName, sid) => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        odds: rate,
        runs: rate,
        bet_market_type: "ROULETTE13",
        oddsmarketId: sid,
        eventManualType: "Auto",
        market_runner_name: selectionName,
        market_odd_name: "ROULETTE13",
        bet_event_name: "ROULETTE13",
      ...
      const data = await placeBetRoulette13Api(params);
      if (isApiSuccess(data)) {
        if (data.last_bet_id) {
    ...
    return (
      <div
        ...
        style={{
          ...
          fontWeight: "bold",
          fontSize: isMobileBoard ? "18px" : "18px",
          border: `1px solid ${isLight ? "#fff" : "#333"}`,
          cursor: isSuspended ? "not-allowed" : "pointer",
          gridArea: gridArea,
    ...
    <>
                ...
                >
                  <a href="/owncasino" style={{ display: "flex" }}>
                    <i
                      ...
                      style={{ color: isLight ? "var(--text-white)" : "#fff", fontSize: "16px" }}
                    ></i>
                  </a>
                </div>
                <div
          ...
          <div className={`casino-video ${isVideoFull ? "full" : ""}`}>
            ...
            <div className="casino-video-right-icons">
              ...
              <div className="casino-video-rules-icon">
                <i className="fas fa-info-circle" onClick={() => { setShowWebRules(true); }}></i>
              </div>
              <div
                  ...
                  return (
                    <div className="rules-body">
                      <div>
                        ...
                        <GameRules normalizedGame={'Roulette'} />
                      </div>
                    </div>
                  );
                })()}
      ...
      >
        ...
        >
          ...
          >
            <span style={{ fontSize: "14px" }}>Roulette Result</span>
            <button
          ...
          >
            <div style={{ marginBottom: "4px" }}>
              Round ID:{" "}
              {selectedResult?.rid || selectedResult?.event_id || selectedResult?.mid || roundId || "N/A"}
            </div>
            <div style={{ marginBottom: "4px" }}>
              Match Time:{" "}
      ...
      <RulesModal
        ...
        gameKey={'Roulette'}   // or gameId
      />
    </>
  );
};

export default Roulette13;
