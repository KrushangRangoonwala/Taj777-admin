            ...
            >
                ...
                >
                    {modalContent?.mainCard ? (
                        <img
                            src={(() => {
                                let code = modalContent.mainCard?.toString().trim().toUpperCase();
                                const map = { A: "ASS", J: "JSS", Q: "QSS", K: "KSS" };
                                code = map[code] || code;
...
export const formatResultData = (result) => {
    ...
    // Check if desc_remakrs uses pipe format (Bollywood Table format)
    const descRemakrs = result.desc_remakrs || "";
    if (descRemakrs.includes("|")) {
        // Parse pipe-delimited format: "Winner | Odd/Even | Color | Dulha/Barati | Card X"
        const parts = descRemakrs.split("|").map(p => p.trim());

        // Extract card letter from "Card K" format
        const cardPart = parts[4] || "";
        const cardValue = cardPart.toLowerCase().startsWith("card")
            ? cardPart.substring(4).trim()
            : cardPart;
        ...
        return {
            winner: parts[0] || "N/A",
            cardDisplay: (parts[1]?.toLowerCase() === "odd") ? "Yes" : parts[1] || "N/A",  // Show Yes if Odd
            underOver: parts[2] || "N/A",   // Color
            oddEven: parts[3] || "N/A",     // Dulha/Barati
            color: cardValue || "N/A",      // Card X

            mainCard: rawMainCard,
            mainCardImage: cardCodeForImage(rawMainCard),
    ...
    // Fallback to hash-delimited format
    const desc = parseDescription(descRemakrs);
