...
const PlacebetUniqueRoulette = ({
  ...
  const loadOpenBets = useCallback(async () => {
    try {
      const res = await fetchOpenBetsApi({
        markettype: "OURROULLETE", // Updated for Roulette
        eventId: "1",
        curPageName: "live_ourroullete.php", // Placeholder page name
      });
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      console.log(
        "✅ PlacebetUniqueRoulette connected to result socket:",
        socket.id
      );
      socket.emit("Room", "ourroullete"); // Updated room name
    ...
    socket.on("gameResult", (data) => {
      ...
      if (data?.res?.length > 0) {
        ...
        socketRef.current.eventData = {
          ...
          gtype: latestResult.gtype || "ourroullete", // Updated game type
        };
      }
  ...
  const handleSpanClick = async (clickedIndex, e) => {
    ...
    try {
      ...
      const response = await fetchTeenpattiResults(
        mid,
        resultItem.gtype || "ourroullete", // Updated game type
        1,
        login_user_id,
        auth_key
      );
  ...
  const handleSubmit = async () => {
    ...
    params.append("eventType", "OURROULLETE");
    ...
    params.append("bet_market_type", "OURROULLETE");
    params.append("oddsmarketId", betData.marketId);
    params.append("eventManualType", "Auto");
    params.append("market_runner_name", teamName);
    params.append("market_odd_name", "OURROULLETE");
    params.append("bet_event_name", "Unique Roulette");
    params.append("bet_type", isBack ? "Back" : "Lay");

    params.append("gtype", "ourroullete");
    ...
    try {
      ...
      const response = await placeBetUniqueRouletteApi(params);
                ...
                {lastResults && lastResults.length > 0 ? (
                  lastResults.map((result, index) => {
                    ...
                    return (
                      ...
                      >
                        {num === 0 ? "0" : num < 10 ? `0${num}` : num}
                      </span>
                    );
                  })
                ) : (
                  <div style={{ color: "#fff", padding: "10px" }}>No results</div>
                )}
              </div>
            </div>
...
export default PlacebetUniqueRoulette;
